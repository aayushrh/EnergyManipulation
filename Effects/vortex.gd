		...
		for i in bodiesIn:
			...
			if i is Blast:
				...
				if i is CharacterBody2D and !hasV:
					timer -= delta
					if(timer <= 0):
						i.commit_dmg(-spell.getPower() * spell.getAttrScaled("Suction Damage") * 2.5 / max(pow(i.global_position.distance_to(global_position) / spell.getSize() * 10,0.5),0.5)/6, {"type": "vortex", "color": Color(0,0,0)})
						timer = tickRate
					if !i.slow:
